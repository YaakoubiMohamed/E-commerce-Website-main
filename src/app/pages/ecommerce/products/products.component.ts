...
export class ProductsComponent implements OnInit {
  ...
  getArticle() {
    this.articlesservice.getArticles().snapshotChanges().subscribe(data => {
      ...
      this.page = Math.ceil(this.articles.length / 2)
  ...
  searchFilter(e) {
    ...
    this.searchArticles = this.articles.filter((article) => {
      return article.nom.toLowerCase().search(searchStr.toLowerCase()) !== -1;
    });
    console.log(this.articles)
  }
